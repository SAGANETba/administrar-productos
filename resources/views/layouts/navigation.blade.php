    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">
                        ...
                        @if (\App\Support\Cart::count() > 0)
                            <span class="ms-1 inline-flex items-center justify-center rounded-full bg-emerald-600 text-white text-xs w-5 h-5">{{ \App\Support\Cart::count() }}</span>
                        @endif
                    </x-nav-link>
                    @auth
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm text-gray-700 hover:text-gray-900 mr-4">{{ __('Iniciar sesión') }}</a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">
                {{ __('Carrito') }} ({{ \App\Support\Cart::count() }})
            </x-responsive-nav-link>
            @auth
